...
const BlogPostPage = () => {
    ...
    const renderContent = (content: string) => {
        return content.split('\n\n').map((paragraph, i) => {
            ...
            if (paragraph.includes('\n• ')) {
                ...
                return (
                    <div key={i} className="my-8">
                        <p className="font-bold text-white mb-4 uppercase tracking-widest text-xs opacity-50">{title}</p>
                        <ul className="space-y-3">
                            {items.map((item, idx) => (
                                <li key={idx} className="flex gap-3 text-zinc-300 leading-relaxed">
            ...
            // Regular paragraph
            return <p key={i} className="whitespace-pre-wrap text-zinc-300 text-lg md:text-xl leading-relaxed">{paragraph}</p>;
        });
    };
